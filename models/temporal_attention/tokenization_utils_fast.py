...
"""
Tokenization class for fast temporal tokenizers.
"""
...
from transformers.tokenization_utils_fast import ADDED_TOKENS_FILE, TOKENIZER_FILE
...
def prepend(texts, time_ids, stringify_time_id=noop):
    """Prepend the time to each text"""
    texts = [
        f"{stringify_time_id(time_id)} {text}" for text, time_id in zip(texts, time_ids)
    ]
    return texts
...
class TempoPreTrainedTokenizerFast(TempoPreTrainedTokenizerBase):
    """
    ...
    def _convert_encoding(
        ...
        return_time_ids: Optional[bool] = None,
        ...
        if return_time_ids is None:
            return_time_ids = "time_ids" in self.model_input_names
        ...
        for e in encodings:
            ...
            if return_time_ids and hasattr(e, "time_ids"):
                encoding_dict["time_ids"].append(e.time_ids)
    ...
    def _batch_encode_plus(
        ...
        batch_time_id: Union[List[str], List[Tuple[str, str]]] = None,
        ...
        if self.time_embedding_type.startswith("prepend") and batch_time_id:
            kwargs = {}
            if self.time_embedding_type.startswith(("prepend_token", "append_token")):
                kwargs["stringify_time_id"] = lambda time_id: f"<{time_id}>"
            if utils.check_type(batch_text_or_text_pairs, List[TextInput]):
                texts = prepend(batch_text_or_text_pairs, batch_time_id, **kwargs)
            elif utils.check_type(batch_text_or_text_pairs, List[TextInputPair]):
                texts = prepend_tuples(
                    batch_text_or_text_pairs, batch_time_id, **kwargs
                )
            else:
                raise 'Unsupported batch_text_or_text_pairs'
        ...
        if utils.is_time_id_necessary(self.time_embedding_type):
            if not batch_time_id:
                raise ValueError("batch_time_id is missing")
            # Add time ids to the encoding (use the same format as of the other properties)
            for seq_i, encoding in enumerate(encodings):
                ...
                # Replace any unknown time_id with the time_id of its nearby token
                for i, time_id in enumerate(encoding.time_ids):
                    if time_id == self.pad_time_id:
                        nearby_index = i + 1 if i == 0 else i - 1
                        encoding.time_ids[i] = encoding.time_ids[nearby_index]
        ...
        tokens_and_encodings = [
            self._convert_encoding(
                ...
                return_time_ids=return_time_ids,
    ...
    def _encode_plus(
        ...
        **kwargs,
        ...
        if not time_id:
            batched_time_id = None
        else:
            batched_time_id = [(time_id, time_id_pair)] if text_pair else [time_id]
